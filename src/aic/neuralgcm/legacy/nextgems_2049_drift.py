#!/usr/bin/env python
"""Drift statistics (year-mean RMSE + bias vs lead time) for NextGEMS-2049.

Analogue of cell 6 of rackow_daily_rollouts.ipynb, verified against NextGEMS-2049
(on the model grid) instead of ERA5. For each init-day d and lead step s:
     diff   = T_pred(d,s) - T_nextgems(valid_time(d,s))   on the model grid
     mse(s) = mean_d < diff^2 >   (cos-lat weighted spatial mean)
     bias(s)= mean_d < diff >
     RMSE(s)= sqrt(mse(s))
Computed for every requested pressure level; one twin-axis figure per level under
figures/drift_stats/.
"""
...
def build_drift(levels, truth) -> pd.DataFrame:
    csv = C.OUTDIR / f"nextgems_{YEAR}_drift_per_init_{C.level_tag()}.csv"
    if csv.exists():
        print(f"[drift] cached {csv}")
        return pd.read_csv(csv, parse_dates=["init_date"])
    files = sorted(C.PRED_DIR.glob(f"pred_{YEAR}_*.nc"))
    print(f"[drift] scoring {len(files)} rollouts vs NextGEMS at {len(levels)} levels")
    rows = []
...
def main():
    ...
    for lev in levels:
        ...
        ax_rmse.set_title(f"{YEAR} — {lev} hPa T vs NextGEMS "

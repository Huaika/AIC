#!/usr/bin/env python
"""Global 10-day temperature drift maps (Rackow et al. 2024 Fig. 3 style) --
RUN-AGNOSTIC, per requested pressure level.
...
Three panels per level (NeuralGCM day-10 clim, reference, drift). Run via EVAL_RUN.
"""
...
def build_fields(levels, truth) -> xr.Dataset:
    nc = C.OUTDIR / f"{C.RUN}_drift_maps_{C.level_tag()}.nc"
    if nc.exists():
...
def main():
    ...
    for lev in levels:
        ...
        fig.suptitle(f"{C.REF_LABEL} — mean 10-day T@{lev} hPa drift "
                     f"(Rackow et al. 2024, Fig. 3 style)", y=1.04, fontsize=13)
        fig.tight_layout()
        out_png = FIGDIR / f"{C.RUN}_driftmap_T_L{lev:04d}.png"

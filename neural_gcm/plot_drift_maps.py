...
def build_fields(var, short, levels, truth) -> xr.Dataset:
    """Global day-10 clim / reference clim / drift, cached once per (run, var)."""
...
def plot_variable(var, levels, regions):
    ...
    for reg in regions:
        figdir = C.figure_dir(reg, var, "drift_maps")
...
def main():
    ...
    print(f"done -> {C.FIGROOT}/<region>/<variable>/drift_maps/")

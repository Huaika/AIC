#!/usr/bin/env python
"""Spaghetti plots (Rackow et al. 2024 Fig. 1 style) -- RUN-AGNOSTIC.

Continuous reference daily-mean global temperature (thick black) + one thin
10-day/6 h rollout line per init-day (collapsed to per-day means), per requested
pressure level. One full-year figure per level (NO January zoom -- removed).

Run selected via EVAL_RUN; see eval_common.py.
"""
...
def build_rollout_gmean(levels) -> pd.DataFrame:
    csv = C.OUTDIR / f"{C.RUN}_rollout_gmean_{C.level_tag()}.csv"
    if csv.exists():
...
def spaghetti(ax, roll_lev, ref_daily_lev, level, color, every=1):
    ...
    ax.set_title(f"Global-mean temperature at {level} hPa — {C.REF_LABEL}")
...
def main():
    ...
    for k, lev in enumerate(levels):
        ...
        fig.tight_layout()
        out = FIGDIR / f"{C.RUN}_spaghetti_T_L{lev:04d}.png"
